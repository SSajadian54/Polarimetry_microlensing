...
####################################  COOL STARS    ###########################################3
numf=int(5);     numm=int(159);      numw=int(20)
norm=np.zeros((numf))            
norm=[340.47576713,780.34789551,986.66688173,1050.56463272,692.7877672]
tranq=np.zeros((numw,numf+1))    
tranq=np.loadtxt('./transmission/transmission_cool.txt')
c=['m','b','g','y','r']
dirname="./Patrick_Harrington/cool/"

filr=open("./prop.txt","w");  
filr.close();
...
#########  saving tramsnission in a file and calculating normalization factor && plotting it   
    fii=open("./transmission/transmission.txt","w");  fii.close()           
    norm=np.zeros((numf))            
    for i in range(numw):            
        d=np.zeros((numf+1))
        fii=open("./transmission/transmission.txt","a")
        d[0]=float(wave[i]); 
        if(i!=int(numw-1)):  up=float(wave[i+1])
        else:                up=float(wave[numw-1])-200  
        for j in range(numf): 
    ...
    fig=plt.gcf()
    fig.savefig("./transmission/transmission1.jpg",dpi=200)
    '''
...
########## reading the rest of file#############################################
    ...
    prop=np.zeros((3))
    prop[0]=Teff;  prop[1]=float(gra[0]);  prop[2]= numm
